  ...
  class Battle
    ...
    def start(is_arena = nil, is_event = nil)
      ...
      loop do
        if monster_dead?
          monster_death
          return
        elsif player_dead? then player_death
        end
        ...
        if world.debug_mode || PlayerLevels::get_level_stats(player.level)[:special_abilities].include?(:rocking_vision)
          print monster.hp_cur.to_s.rjust(3).to_s
        else print "???"
        end
        print " HP"
        ...
        when 'fight', 'f', 'attack', 'a'
          ...
          else puts "You miss entirely!".colorize :yellow 
          end
        when 'defend', 'd'
          puts 'You dig in and defend this round.'
          self.player_defending = true
        when 'look', 'l'
          print monster.name.to_s.colorize :white
    ...
    def monster_attacks_player
      ...
      if dmg > 0
        ...
        player_death if player.dead?
      else puts "#{monster.name} misses entirely!".colorize :yellow 
      end
    end
    ...
    def update_player_stats
      ...
      player.rox += monster.rox
      
      monster_items = monster.inventory.items
      player.inventory.items.concat monster_items unless monster_items.empty? unless monster_items.nil?
    ...
    def player_escape?
      if (player.dexterity > monster.dexterity) then return true
      else
